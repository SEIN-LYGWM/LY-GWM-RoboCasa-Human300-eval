...
# M489_S41_REWARD_ALIGNMENT_TARGET50_BATCHED_V1
import hashlib
import signal
import subprocess
import time
...
from decision_audit import verify_assets, verify_evidence, action_sha
JOINT = verify_assets(MASTER, PROTOCOL, verify_policy_weights=ROLE == 'orchestrate')
TASK_INDEX = int(os.environ["SLURM_ARRAY_TASK_ID"])
if TASK_INDEX not in PROTOCOL["experiment"]["task_indices"]:
    raise RuntimeError("Task not included in this experiment")
...
def server(report):
    ...
    from decision_adapter import LYDecisionAdapter
    ...
    with torch.no_grad():
        policy = Gr00tPolicy(
            ...
            modality_transform=config.transform(), denoising_steps=4, device="cuda")
    adapter = LYDecisionAdapter(policy, JOINT['lygwm_checkpoint'],
                                JOINT['lygwm_checkpoint_sha256'], JOINT['mode'], TASK_INDEX,
                                JOINT['scorer_checkpoint'], JOINT['scorer_checkpoint_sha256'],
                                JOINT['scorer_config_sha256'])
    adapter.install()
    report.update(model_loaded=True, lygwm_loaded=True, scorer_loaded=True,
                  scorer_checkpoint_sha256=adapter.scorer_sha256,
                  lygwm_checkpoint_sha256=JOINT['lygwm_checkpoint_sha256'],
                  decision_mode=JOINT['mode'], candidate_count=4,
                  prediction_used_for_selection=JOINT['mode']=='learned_reward',
                  successful_requests=0, handler_errors=0, decision_samples=0,
                  selected_nonfirst_chunks=0, changed_environment_chunks=0,
    ...
    trace = (RUN / ('selection-' + JOB + '.jsonl')).open('x', encoding='utf-8')
    def get_action(observation):
        try:
            observation = dict(observation)
            context = observation.pop('__lygwm_context__')
            assert isinstance(context,list) and 0<len(context)<=5
            assert all(c['task_index']==TASK_INDEX for c in context)
            adapter.context = context
            with torch.inference_mode():
                action = policy.get_action(observation)
                first_action = policy._get_unnormalized_action(adapter.last_first_action.clone())
            torch.cuda.synchronize()
            report['successful_requests'] += 1
            assert adapter.calls==report['successful_requests']
            evidence = adapter.last_evidence
            evidence.update(protocol_sha256=PROTOCOL_SHA,task_index=TASK_INDEX,job_id=JOB)
            assert len(evidence['rows'])==len(context)
            for i,row in enumerate(evidence['rows']):
                selected={key:value[i] for key,value in action.items()}
                original={key:value[i] for key,value in first_action.items()}
                assert set(selected)==set(original)
                delta=max(float(np.max(np.abs(selected[k]-original[k]))) for k in selected)
                assert np.isfinite(delta)
                row.update(environment_action_sha256=action_sha(selected),
                           first_environment_action_sha256=action_sha(original),
                           environment_action_max_abs=delta,environment_action_changed=delta>0)
                if JOINT['mode']=='first_candidate':
                    assert delta==0, 'Control changed the first candidate'
                if adapter.calls==1:
                    row['selected_environment_action']={k:v.tolist() for k,v in selected.items()}
                report['selected_nonfirst_chunks']+=int(row['selected_index']!=0)
                report['changed_environment_chunks']+=int(delta>0)
            report['decision_samples']+=len(context)
            trace.write(json.dumps(evidence,sort_keys=True,allow_nan=False)+'\n')
            trace.flush()
            if adapter.calls==1 or adapter.calls%25==0:
                save('server-progress',report)
                log('DECISION_PROGRESS '+json.dumps({k:report[k] for k in
                    ('successful_requests','decision_samples','selected_nonfirst_chunks','changed_environment_chunks')}))
            return action
        except Exception:
    ...
    try:
        ...
        ENDPOINT.write_text(json.dumps(dict(job_id=JOB,host='127.0.0.1',port=int(address.rsplit(':',1)[1]))),encoding='utf-8')
        endpoint_server.run()
        report['accepted']=(report['successful_requests']>0 and report['handler_errors']==0
                            and adapter.calls==report['successful_requests'])
...
def simulation(report):
    ...
    try:
        ...
        while pending or live:
            ...
            batch['__lygwm_context__'] = [dict(task_index=TASK_INDEX,
                episode_index=slot['index'], request_index=slot['requests'],
                env_step_count=len(slot['env'].reward)) for slot in live]
            ...
            for batch_index, slot in enumerate(live):
                ...
                received_sha = action_sha(action)
                request_index = slot['requests']
                ...
                with (RUN / ('execution-' + JOB + '.jsonl')).open('a', encoding='utf-8') as stream:
                    stream.write(json.dumps(dict(protocol_sha256=PROTOCOL_SHA,
                        episode_index=slot['index'], request_index=request_index,
                        received_action_sha256=received_sha, executed_steps=executed,
                        executed_prefix_sha256=action_sha({k:v[:executed] for k,v in action.items()})),
                        sort_keys=True)+'\n')
                # Raw per-step rewards belong to the reached states. The wrapper's
                # returned reward aggregates history and is NOT the t+16 label.
                reward_sequence = [float(v) for v in env.reward[before:after]]
                assert len(reward_sequence)==executed
                assert all(v in (0.0,1.0) for v in reward_sequence)
                assert np.array_equal(np.asarray(reward_sequence),np.asarray(info['rewards']).reshape(-1))
                with (RUN / ('reward-observations-' + JOB + '.jsonl')).open('a', encoding='utf-8') as stream:
                    stream.write(json.dumps(dict(protocol_sha256=PROTOCOL_SHA,
                        task_index=TASK_INDEX,episode_index=slot['index'],request_index=request_index,
                        source_env_step_count=before,target_env_step_count=after,
                        executed_steps=executed,received_action_sha256=received_sha,
                        endpoint_reward=reward_sequence[-1],per_step_rewards=reward_sequence,
                        chunk_any_reward=bool(max(reward_sequence)),
                        terminated=bool(terminated),truncated=bool(truncated)),sort_keys=True)+'\n')
                slot["obs"] = obs
                slot["requests"] += 1
                ...
                if bool(terminated) or bool(truncated):
                    ...
                    print(
                        ...
                        "steps=%d valid=%d (see planned episode_count) successes=%d"
                        % (
            ...
            if report["successful_requests"] % 10 == 0:
                print(
                    "progress task=%s batch_requests=%d valid=%d (see planned episode_count) live_steps=%s"
                    % (
                        TASK["task_name"],
...
def orchestrate(report):
    ...
    try:
        ...
        if results:
            raise RuntimeError('Partial task exists; use a new experiment directory, do not silently resume')
        task_summary(results)
        ...
        simulation_rc = simulation_process.wait(timeout=82800)
        ...
        assert server_report['lygwm_loaded'] is True
        assert server_report['scorer_loaded'] is True
        assert server_report['scorer_checkpoint_sha256'] == JOINT['scorer_checkpoint_sha256']
        assert server_report['decision_mode'] == JOINT['mode']
        assert server_report['lygwm_checkpoint_sha256'] == JOINT['lygwm_checkpoint_sha256']
        evidence = verify_evidence(RUN, JOB, PROTOCOL_SHA, JOINT['mode'])
        assert evidence['request_batches'] == sim_report['successful_requests']
        assert evidence['executed_action_chunks'] == server_report['decision_samples']
        report.update(decision_evidence=evidence, decision_mode=JOINT['mode'])

        results = existing_results()
        if len(results) != TASK["episode_count"]:
            raise RuntimeError("Expected planned episode records")
...
report = {
    ...
    "formal_target50_evaluation": PROTOCOL["experiment"]["phase"] == "full",
    "split": "pretrain",
...
save(name, report)
log("##### COMPACT_REPORT_BEGIN: M489_S41_REWARD_ALIGNMENT_" + ROLE.upper() + " #####")
log(json.dumps(report, ensure_ascii=False, indent=2))
log("##### COMPACT_REPORT_END: M489_S41_REWARD_ALIGNMENT_" + ROLE.upper() + " #####")

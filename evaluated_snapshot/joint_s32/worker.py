...
# M489_S32_JOINT_TARGET50_BATCHED_V1
import hashlib
import signal
import subprocess
import time
...
from joint_audit import verify_assets
JOINT = verify_assets(MASTER, PROTOCOL)
...
def server(report):
    ...
    # This exact adapter passed W6. Its constructor preserves the policy RNG.
    adapter = LYShadowAdapter(policy, JOINT['lygwm_checkpoint'], JOINT['lygwm_checkpoint_sha256'])
    adapter.install()
    report.update(lygwm_loaded=True, lygwm_checkpoint_sha256=JOINT['lygwm_checkpoint_sha256'],
                  shadow_outputs_influence_actions=False, shadow_forward_count=0,
                  shadow_prediction_samples=0, shadow_graph_seconds_total=0.0)
    ...
    def get_action(observation):
        ...
        try:
            ...
            assert adapter.calls == report['successful_requests'], 'Shadow did not run exactly once'
            prediction = adapter.last_prediction
            assert prediction is not None
            assert all(torch.isfinite(v).all() for v in prediction.values())
            batch_size = int(prediction['state'].shape[0])
            evidence = dict(call=adapter.calls, batch_size=batch_size, finite=True,
                            feature_shape=list(prediction['features'].shape),
                            state_shape=list(prediction['state'].shape),
                            graph_seconds=adapter.last_graph_seconds,
                            feature_mean=float(prediction['features'].mean().item()),
                            state_mean=float(prediction['state'].mean().item()))
            report.setdefault('first_shadow_prediction', evidence)
            report['last_shadow_prediction'] = evidence
            report['shadow_forward_count'] = adapter.calls
            report['shadow_prediction_samples'] += batch_size
            report['shadow_graph_seconds_total'] += adapter.last_graph_seconds
            if adapter.calls == 1 or adapter.calls % 50 == 0:
                save('server-progress', report)
    ...
    try:
        ...
        report["accepted"] = (
            ...
            and report['shadow_forward_count'] == report['successful_requests']
            and report['shadow_prediction_samples'] > 0
        )
...
def orchestrate(report):
    ...
    try:
        ...
        assert server_report['shadow_forward_count'] == sim_report['successful_requests']
        assert server_report['shadow_outputs_influence_actions'] is False
        report.update(joint_shadow_accepted=True,
                      shadow_forward_count=server_report['shadow_forward_count'],
                      shadow_prediction_samples=server_report['shadow_prediction_samples'],
...
save(name, report)
log("##### COMPACT_REPORT_BEGIN: M489_S32_JOINT_" + ROLE.upper() + " #####")
log(json.dumps(report, ensure_ascii=False, indent=2))
log("##### COMPACT_REPORT_END: M489_S32_JOINT_" + ROLE.upper() + " #####")
